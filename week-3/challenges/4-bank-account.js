...
const bankAccount = {
  ...
  const donateMoney = (amount, onSuccess, onFail) => {
    if(bankAccount.currentBalance >= amount){
        ...
        bankAccount.transactions.push({prevAmount : prevAmount , newAmount : bankAccount.currentBalance , reason : "Donation"});
        onSuccess();
    } else {
        onFail();
    }
...
//   default result
  ...
  const donateMoney1 = (amount, onSuccess, onFail) => {
    createPayment(amount, onSuccess, onFail, "Donation");
  };
  const payRent1 = (amount, onSuccess, onFail) => {
